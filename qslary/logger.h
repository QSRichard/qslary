...
#define QSLARY_LOG_LEVEL(logger, level) \
  ...
  qslary::LogEventWarp(qslary::LogEvent::ptr(new qslary::LogEvent(logger, level, __FILE__, __LINE__, 0, qslary::detail::getThreadId(), qslary::detail::getFiberId(), time(0),qslary::CurrentThread::name()))).getSS()
...
namespace qslary
{
  ...
  public:
    ...
    LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, uint32_t line, uint32_t elpase, uint32_t threadId, uint32_t fiberId, uint64_t time,const std::string& thread_name);
