...
Socket::Socket(int family, int type, int protocol)
    : socketfd_(-1), family_(family), type_(type), protocol_(protocol),
      isConnected_(false)
{}
...
Socket::ptr Socket::accept()
{
    ...
    {
        QSLARY_LOG_ERROR(g_logger)
            << "accept( " << socketfd_ << " ) errno = " << errno << " errstr "
            << strerror(errno);
        return nullptr;
    }
    if (sock->init(newfd))
...
bool Socket::bind(const Address::ptr addr)
{
    ...
    {
        QSLARY_LOG_ERROR(g_logger) << "::bind error errno " << errno
...
int Socket::recvFrom(void *buffer, size_t length, Address::ptr from, int flags)
{
    if (isConnected())
    {
        // revcfrom 的最后一个参数需要一个socklen_t 的地址
        // 因此需要在此定义一个变量
...
Address::ptr Socket::getRemoteAddress()
{
    ...
    {
        QSLARY_LOG_ERROR(g_logger)
            << "getpeername error sock = " << socketfd_ << " errno " << errno
...
Address::ptr Socket::getLocalAddress()
{
    ...
    {
        QSLARY_LOG_ERROR(g_logger)
            << "getsockname error sock = " << socketfd_ << " errno " << errno
            << " str errno " << strerror(errno);
        return nullptr;
    }
    if (family_ == AF_UNIX)
...
// {
//   qslary::FdContext::ptr fd_ctx =
//   qslary::FdMgr::getInstance()->getFdContext(socketfd_); assert(fd_ctx);
...
bool Socket::isVaild() const { return socketfd_ != -1; }
...
{
    os << " Socket sock= " << socketfd_ << " is connect= " << isConnected_
       << " family = " << family_ << " type " << type_ << " protocol "
       << protocol_;
    if (localAddress_)
...
{
    return IOManager::GetIOManager()->CancelAll(socketfd_);
}
