...
ByteArray::ByteArray(size_t base_size)
    : m_baseSize(base_size), m_position(0), m_capacity(base_size), m_size(0), m_endian(QSLARY_BIG_ENDIAN),
...
bool ByteArray::isLittleEndian() const { return m_endian == QSLARY_LITTLE_ENDIAN; }

void ByteArray::setIsLittleEndian(bool val)
{
  if (val)
  {
    m_endian = QSLARY_LITTLE_ENDIAN;
  }
  else
  {
    m_endian = QSLARY_BIG_ENDIAN;
  }
}
...
{
  if (m_endian != QSLARY_BYTE_ORDER)
  {
    value = qslary::byteswap(value);
  }
