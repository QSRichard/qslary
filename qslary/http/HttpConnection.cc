...
HttpConnection::HttpConnection(Socket::ptr sock, bool ower)
    : SockStream(sock, ower)
{}
...
HttpResponse::ptr HttpConnection::recvResponse(HttpResponse::ptr &r)
{
    ...
    if (client_parser.chunked)
    {
        ...
        do
        {
            ...
            //   SYLAR_LOG_DEBUG(g_logger) << "content_len=" <<
            //   client_parser.content_len;
            ...
            else
            {
                ...
                while (left > 0)
                {
                    int rt = read(data, left > (int)buff_size ? (int)buff_size
                                                              : left);
                    if (rt <= 0)
                    {
                        close();
    ...
    //   delete[] ptr; }); char* data = buffer.get(); int offset = 0; do
    //   {
    //     int len = read(data + offset, buff_size - offset);
    //     if (len <= 0)
    //     {
    ...
    //       std::cout << "+++++++++++++++++++++++++++++++" << std::endl;
    //       //   SYLAR_LOG_DEBUG(g_logger) << "content_len=" <<
    //       client_parser.content_len; if (client_parser.content_len + 2 <=
    //       len)
    //       {
    //         std::cout << "=============================" << std::endl;
    //         body.append(data, client_parser.content_len);
    ...
    //         client_parser.content_len - 2); len -= client_parser.content_len
    //         + 2; std::cout << "=============================" << std::endl;
    //       }
    //       else
    //       {
    //         std::cout << "++++-------------------------------++" <<
    //         std::endl; body.append(data, len); int left =
    //         client_parser.content_len - len + 2; while (left > 0)
    //         {
    //           int rt = read(data, left > (int)buff_size ? (int)buff_size :
    //           left); if (rt <= 0)
    //           {
...
HttpResult::ptr
HttpConnection::DoGet(const std::string &url, uint64_t timeout,
...
HttpResult::ptr
HttpConnection::DoGet(qslary::URI::ptr url, uint64_t timeout,
                      const std::map<std::string, std::string> &headers,
                      const std::string &body)
{
...
HttpResult::ptr
HttpConnection::DoPost(const std::string &url, uint64_t timeout,
                       const std::map<std::string, std::string> &headers,
                       const std::string &body)
{
...
HttpResult::ptr
HttpConnection::DoPost(qslary::URI::ptr url, uint64_t timeout,
                       const std::map<std::string, std::string> &headers,
                       const std::string &body)
{
...
HttpResult::ptr HttpConnection::DoRequest(
    HttpMethod method, const std::string &url, uint64_t timeout,
...
HttpResult::ptr HttpConnection::DoRequest(
    HttpMethod method, qslary::URI::ptr uri, uint64_t timeout,
    const std::map<std::string, std::string> &headers, const std::string &body)
{
                                          ...
                                          uint64_t timeout)
{
    ...
    {
        return std::make_shared<HttpResult>(
            (int)HttpResult::Error::CONNECTION_FALID, nullptr,
...
HttpConnectionPool::HttpConnectionPool(const std::string &host,
    ...
    : host_(host), vhost_(vhost), port_(port ? port : (is_https ? 443 : 80)),
      maxSize_(max_size), maxAliveTime_(max_alive_time),
...
{
    // ++ptr->m_request;
    if (!ptr->isConnected() || ((ptr->create_time + pool->maxAliveTime_) >=
                                qslary::detail::GetCurrentMs()))
    {
        delete ptr;
        --pool->total_;
        return;
...
// HttpConnectionPool::HttpConnectionPool(const std::string& host, const
// std::string& vhost, uint32_t port, uint32_t maxSize,
//                                        uint32_t maxAliveTime, uint32_t
//                                        maxRequest)
//     : host_(host), vhost_(vhost), port_(port), maxSize_(maxSize),
//     maxAliveTime_(maxAliveTime), maxRequest_(maxRequest)
// {
// }
...
// HttpResult::ptr HttpConnectionPool::doGet(const std::string& url, uint64_t
// timeout,
...
// HttpResult::ptr HttpConnectionPool::doGet(qslary::URI::ptr uri, uint64_t
// timeout,
//                                           const std::map<std::string,
//                                           std::string>& headers, const
//                                           std::string& body)
...
// HttpResult::ptr HttpConnectionPool::doPost(const std::string& url, uint64_t
// timeout,
//                                            const std::map<std::string,
//                                            std::string>& headers, const
//                                            std::string& body)
...
// HttpResult::ptr HttpConnectionPool::doPost(qslary::URI::ptr uri, uint64_t
// timeout,
//                                            const std::map<std::string,
//                                            std::string>& headers, const
//                                            std::string& body)
...
// HttpResult::ptr HttpConnectionPool::doRequest(HttpMethod method, const
// std::string& uri, uint64_t timeout,
//                                               const std::map<std::string,
//                                               std::string>& headers, const
//                                               std::string& body)
// {
...
// qslary::URI::ptr uri, uint64_t timeout,
//                                               const std::map<std::string,
//                                               std::string>& headers, const
//                                               std::string& body)
// {
...
// HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req, uint64_t
// timeout)
// {
...
//   {
//     return
//     std::make_shared<HttpResult>((int)HttpResult::Error::POLL_GET_CONNECTION,
//     nullptr,
//                                         "Poll Host:" + host_ + "Port: " +
//                                         std::to_string(port_));
//   }
...
// void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool*
// poll)
// {

//   if (!ptr->isConnected()
//             || ((ptr->create_time + poll->maxAliveTime_) >=
//             qslary::detail::GetCurrentMs()))
//   {
//     delete ptr;
//     --poll->maxSize_;
