...
// 从from类型转为to类型
template <class From, class To> class LexicalCast
{
public:
    To operator()(const From &val) { return boost::lexical_cast<To>(val); }
};
...
public:
    typename std::unordered_map<std::string, T> operator()(const std::string &v)
    {
        ...
        for (auto it = node.begin(); it != node.end(); it++)
        {
            ss.str("");
            ss << it->second;
            unordered_map.insert(std::make_pair(
                it->first.Scalar(), LexicalCast<std::string, T>()(ss.str())));
        }
...
public:
    ...
    const T getValue()
    {
        qslary::ReadLockGuard lock(rwMutex_);
        return m_val;
    };
    ...
    void setValue(const T &v)
    {
        ...
        qslary::WriteLockGuard lock(rwMutex_);
        m_val = v;
    };
    ...
    virtual std::string toString() override
    {
        ...
        catch (std::exception &e)
        {
            QSLARY_LOG_ERROR(QSLARY_LOG_ROOT())
                << "Config::toString exception" << e.what()
                << "convert: " << DEMANGLED_CLASS_NAME(&m_val)
                << " to std::string ";
        }
        return "";
    };
    ...
    virtual bool formString(const std::string &val) override
    {
        ...
        {
            QSLARY_LOG_ERROR(QSLARY_LOG_ROOT())
                << "Config::fromString exception " << e.what()
                << " convert string to " << DEMANGLED_CLASS_NAME(&m_val);
        }
        return false;
    }
...
public:
           ...
           const std::string &description = "")
    {
        ...
        qslary::WriteLockGuard lock(GetMutex());
        ...
        if (it != GetDatas().end())
        {
            ...
            if (tmp)
            {
                QSLARY_LOG_INFO(QSLARY_LOG_ROOT())
                    << "Lookup name = " << name << " is exists";
                return tmp;
            }
            else
            {
                QSLARY_LOG_ERROR(QSLARY_LOG_ROOT())
                    << "Lookup name = " << name << " is exists but type is not "
                    << typeid(T).name() << " real type is "
                    << it->second->getTypeName() << " value is "
                    << it->second->toString();
                return nullptr;
            }
        }
        ...
        // if(it){
        //     QSLARY_LOG_INFO(QSLARY_LOG_ROOT())<<"Lookup name = "<<name<<" is
        //     exists"; return it;
        // }
        if (name.find_first_not_of("abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQ"
                                   "RSTUVWXYZ_.0123456789") !=
            std::string::npos)
        {
            QSLARY_LOG_ERROR(QSLARY_LOG_ROOT())
                << "Lookup name is invalid" << name;
...
private:
    ...
    static qslary::RWMutex &GetMutex()
    {
        static qslary::RWMutex rwMutex_;
        return rwMutex_;
    }
};
} // namespace qslary
