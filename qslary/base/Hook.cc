...
struct _HookIniter
{
  _HookIniter()
  {
    ...
    g_tcp_connect_timeout->addListener([](const int& old_value, const int& new_value) {
      QSLARY_LOG_INFO(g_logger) << "tcp connect timeout update"<<"new value "<<new_value;
...
retry:
  ...
  if (n == -1 && errno == EAGAIN)
  {
    ...
    if (to != (uint64_t)-1)
    {
      timer = iomanager->addConditionTimer(to, [winfo, fd, iomanager, event]() {
        auto t = winfo.lock();
        if (!t || t->cancelled)
          return;
    ...
    if (rt)
    {
      QSLARY_LOG_INFO(g_logger) << hook_fun_name << " ===================addEvent(" << fdctx << "," << event << ")";
      if (timer)
      {
        timer->cancel();
      }
...
#undef XX
  ...
  unsigned int sleep(unsigned int seconds)
  {
    ...
    qslary::IOManager* iomanager = qslary::IOManager::GetIOManager();
    printf("\n=========hook sleep =========\n");
    iomanager->addTimer(seconds * 1000, [iomanager, fiber]() { iomanager->scheduler(fiber); });
    qslary::Fiber::YieldToHold();
    return 0;
  }
  ...
  int usleep(useconds_t usec)
  {
    ...
    qslary::Fiber::ptr fiber = qslary::Fiber::GetThreadCurrentFiber();
    qslary::IOManager* iomanager = qslary::IOManager::GetIOManager();
    iomanager->addTimer(usec / 1000, [iomanager, fiber]() { iomanager->scheduler(fiber); });
    printf("\n=========hook usleep =========\n");
    qslary::Fiber::YieldToHold();
    return 0;
  }
  ...
  int nanaosleep(const struct timespec* req, struct timespec* res)
  {
    ...
    qslary::Fiber::ptr fiber = qslary::Fiber::GetThreadCurrentFiber();
    qslary::IOManager* iomanager = qslary::IOManager::GetIOManager();
    iomanager->addTimer(timeout_ms, [iomanager, fiber]() { iomanager->scheduler(fiber); });
    qslary::Fiber::YieldToHold();
    return 0;
  }
  ...
  int connect_with_timeout(int sockfd,const struct sockaddr* addr,socklen_t addrlen,uint64_t timeout_ms)
  {
    ...
    qslary::FdContext::ptr fdctx = qslary::FdMgr::getInstance()->getFdContext(sockfd);
    if (!fdctx)
    {
    ...
    {
      timer = iomanager->addConditionTimer(timeout_ms, [winfo, sockfd, iomanager]() {
        auto t = winfo.lock();
        printf("\n\n==================执行了timer的条件超时方法\n\n");
        if (!t || t->cancelled)
        {
  ...
  {
    int fd = do_io(s, accept_f, "accept", qslary::IOManager::Event::READ, SO_RCVTIMEO, addr, addrlen);
    if (fd >= 0)
      qslary::FdMgr::getInstance()->getFdContext(fd,true);
    return fd;
  }
  ...
  {
    return do_io(fd, read_f, "read", qslary::IOManager::Event::READ, SO_RCVTIMEO, buf, count);
  }

  ssize_t readv(int fd, const struct iovec* iov, int iovcnt)
  {
    return do_io(fd, readv_f, "readv", qslary::IOManager::Event::READ, SO_RCVTIMEO, iov, iovcnt);
  }

  ssize_t recv(int sockfd, void* buf, size_t len, int flags)
  {
    return do_io(sockfd, recv_f, "recv", qslary::IOManager::Event::READ, SO_RCVTIMEO, buf, len, flags);
  }

  ssize_t recvfrom(int sockfd, void* buf, size_t len, int flags, struct sockaddr* src_addr, socklen_t* addrlen)
  {
    return do_io(sockfd, recvfrom_f, "recvfrom", qslary::IOManager::Event::READ, SO_RCVTIMEO, buf, len, flags, src_addr,
                 addrlen);
  }

  ssize_t recvmsg(int sockfd, struct msghdr* msg, int flags)
  {
    return do_io(sockfd, recvmsg_f, "recvmsg", qslary::IOManager::Event::READ, SO_RCVTIMEO, msg, flags);
  }

  ssize_t write(int fd, const void* buf, size_t count)
  {
    return do_io(fd, write_f, "write", qslary::IOManager::Event::WRITE, SO_SNDTIMEO, buf, count);
  }

  ssize_t writev(int fd, const struct iovec* iov, int iovcnt)
  {
    return do_io(fd, writev_f, "writev", qslary::IOManager::Event::WRITE, SO_SNDTIMEO, iov, iovcnt);
  }

  ssize_t send(int s, const void* msg, size_t len, int flags)
  {
    return do_io(s, send_f, "send", qslary::IOManager::Event::WRITE, SO_SNDTIMEO, msg, len, flags);
  }

  ssize_t sendto(int s, const void* msg, size_t len, int flags, const struct sockaddr* to, socklen_t tolen)
  {
    return do_io(s, sendto_f, "sendto", qslary::IOManager::Event::WRITE, SO_SNDTIMEO, msg, len, flags, to, tolen);
  }

  ssize_t sendmsg(int s, const struct msghdr* msg, int flags)
  {
    return do_io(s, sendmsg_f, "sendmsg", qslary::IOManager::Event::WRITE, SO_SNDTIMEO, msg, flags);
  }
