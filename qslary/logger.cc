...
#undef XX
  ...
  LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, uint32_t line, uint32_t elpase, uint32_t threadId, uint32_t fiberId, uint64_t time,const std::string& thread_name)
      : m_file(file), m_line(line), m_elpase(elpase), m_thread_id(threadId), m_fiber_id(fiberId), m_time(time),thread_name_(thread_name),m_logger(logger), m_level(level){};
  ...
  {
    // TAG LoggerFormatter default          // 时间 Tab 线程ID Tab 线程名 Tab 协程ID Tab Level Tab Logger Tab File:line Tab message \n
...
#undef XX
  ...
  // TAG 在这里执行了lookup()函数 会在GetData()得到的map<string,ConfigBase&>中加入一个entry
                    ...
                    for(auto& a:i.appenders){
                        ...
                        if(!a.formatter.empty()){
                            ...
                            if(!fmt->isError()){
                                ...
                                std::cout<<"logger.name = "<<i.name<<" appender type = "<<a.type
                                                           <<" formatter = "<<a.formatter<<"is invalib"<<std::endl;
                            }
                        }
  ...
  // TAG 全局静态变量 会在这里执行构造函数 在构造函数中对 g_log_defines加入一个监听器 监听器会在setValue()函数中被调用
