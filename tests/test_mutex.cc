...
int main(int argc, char **argv)
{
    ...
    // for(int i=0;i<5;i++){
    //     qslary::Thread::ptr pth(new
    //     qslary::Thread(fun1,"name_"+std::to_string(i)));
    ...
    YAML::Node root =
        YAML::LoadFile("/home/liushui/workspace/qslary/bin/config/log2.yml");
    qslary::Config::loadFromYaml(root);

    for (int i = 0; i < 2; i++)
    {
        qslary::Thread::ptr pth1(
            new qslary::Thread(fun2, "name_" + std::to_string(i)));
        qslary::Thread::ptr pth2(
            new qslary::Thread(fun3, "name_" + std::to_string(i)));
