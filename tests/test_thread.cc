...
void fun1()
{
    QSLARY_LOG_INFO(g_logger)
        << "name: " << qslary::CurrentThread::name() << " tid "
        << qslary::CurrentThread::tid() << " is main thread "
        << qslary::CurrentThread::isMainThread();
}
...
int main(int argc, char **argv)
{
    ...
    QSLARY_LOG_INFO(g_logger)
        << " main thread " << qslary::CurrentThread::tid();
    QSLARY_LOG_INFO(g_logger)
        << " main thread " << qslary::CurrentThread::name();
